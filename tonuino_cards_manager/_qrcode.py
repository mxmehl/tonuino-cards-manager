...
def generate_qr_codes(qrdata: list[str], maxcardsperqrcode: int):
    ...
    for idx, qrlist in enumerate([qrdata[x : x + maxcardsperqrcode] for x in range(0, len(qrdata), maxcardsperqrcode)]):
        qrc = QRCode()
        qrc.add_data("\n".join(qrlist))
        print(
            f"QR code for cards batch {idx + 1} "
            f"(cards {(idx * maxcardsperqrcode) + 1} - {min((idx + 1) * maxcardsperqrcode,len(qrdata))}):"
        )
        qrc.print_ascii()
